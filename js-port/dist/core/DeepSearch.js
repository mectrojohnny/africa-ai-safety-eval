...
exports.DeepSearch = void 0;
const zod_1 = require("zod");
const dotenv_1 = __importDefault(require("dotenv"));
const ResearchProgress_1 = require("./ResearchProgress");
 ...
 */
const ResearchParametersSchema = zod_1.z.object({
    breadth: zod_1.z.number().int().min(1).max(10),
 ...
 */
const FollowUpQuestionsSchema = zod_1.z.object({
    follow_up_queries: zod_1.z.array(zod_1.z.string()),
});
/**
 * Query response schema
 */
const QueryResponseSchema = zod_1.z.object({
...
const LearningSchema = zod_1.z.object({
    ...
    relevance: zod_1.z.number().min(0).max(10).optional(),
 ...
 */
const ResearchResponseSchema = zod_1.z.object({
...
class DeepSearch {
     ...
     */
    constructor(genAI, mode = 'balanced') {
        this.progress = null;
        this.genAI = genAI;
        this.mode = mode;
        this.model = this.genAI.getGenerativeModel({ model: 'gemini-2.0-flash' });
        this.queryHistory = new Set();
        ...
        this.rateLimiter = new RateLimiter_1.RateLimiter(rateLimiterDelay);
    }
    /**
     ...
     */
    async determineResearchBreadthAndDepth(query) {
        const userPrompt = `
     ...
     */
    async generateFollowUpQuestions(query, parameters, parentQuery) {
     ...
     */
    async generateQueries(query, numQueries, previousQueries = []) {
        const prompt = `
    ...
    `;
        ...
        try {
            ...
            try {
                ...
                const filteredQueries = validated.queries.filter(q => !this.isSimilarToExistingQueries(q, previousQueries));
     ...
     */
    async researchSingleQuery(query, includeSourceLinks) {
        const prompt = `
     ...
     */
    isSimilarToExistingQueries(query, existingQueries) {
     ...
     * @param includeSourceLinks Whether to include source links
     */
    async processQuery(query, currentDepth, maxDepth, maxBreadth, includeSourceLinks) {
        if (!this.progress) {
            this.progress = new ResearchProgress_1.ResearchProgress(query);
        }
        ...
        const params = { breadth: maxBreadth, depth: maxDepth };
     ...
     */
    async deepResearch(topic, maxDepth = 3, maxBreadth = 3, includeSourceLinks = true) {
        this.progress = new ResearchProgress_1.ResearchProgress(topic);
     ...
     */
    async generateFinalReport(researchProgress, format = 'markdown') {
      ...
      Sources:
      ${Object.values(visitedUrls).map((source, i) => `${i + 1}. ${source.title || 'Source'} - ${source.url}`).join('\n')}
      ` : ''}
