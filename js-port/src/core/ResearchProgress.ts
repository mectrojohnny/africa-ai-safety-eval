...
export class ResearchProgress {
   ...
   */
  private extractLearnings(answer: string): void {
   ...
   */
  private isDuplicateLearning(learning: string): boolean {
    // Simple duplicate check - can be enhanced with similarity detection
    return this.learnings.some(existing => {
      // Exact match
      if (existing === learning) return true;
      
      // Significant overlap (contained within each other)
      if (existing.includes(learning) || learning.includes(existing)) return true;
   ...
   */
  private addSources(sources: Source[]): void {
